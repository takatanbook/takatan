...
public class UtilValidator {
		...
		public static final String EIMOZI         = "[a-zA-Z]+$";
		
		private static final String MOZI = "[^a-zA-Zぁ-んァ-ヶ一-龠々ー]";
		
		private static final Pattern P_NAME = Pattern.compile(MOZI);
		 
		private static final String P_STR = "[^0-9a-zA-Zぁ-んァ-ヶ一-龠々ー]";
	    private static final Pattern P_VALUE = Pattern.compile(P_STR);
		// チェック処理
		public boolean isEnAndJp(String name) {
	    ...
	    public boolean isEnAndJpAndNumber(String name) {
	    ...
	    public boolean isMatch2(String s) {
	        Matcher matcher =  P_VALUE.matcher(s);
	        return matcher.find();
	    }
		...
		public boolean jpEnCodeCheck(String name){
			boolean msg = false;
			//平仮名かカタカナを正規表現で一致するか（カタカナとひらがなの正規表現を合体）
			if (name.matches(toMatchRegex(P_HIRAGANA_ONLY +P_KANG_ONLY + EIMOZI))) {
				msg = true;
			}
			return msg;
		}
